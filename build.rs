...
fn main() {
    ...
    println!("cargo:rerun-if-changed=proto/raft/common.proto");
    println!("cargo:rerun-if-changed=proto/raft/meta.proto");
    println!("cargo:rerun-if-changed=proto/raft/data.proto");
...
fn compile_raft_proto() {
    let raft_proto = Path::new("proto/raft/raft.proto");
    if !raft_proto.exists() {
        println!("cargo:warning=raft proto not found, skip compile: {}", raft_proto.display());
        write_empty_raft_rs();
        return;
    }

    // During scaffold stage proto files can be empty placeholders.
    let is_empty = fs::metadata(raft_proto)
        .map(|m| m.len() == 0)
        .unwrap_or(true);
    if is_empty {
        println!("cargo:warning=raft proto is empty, skip compile: {}", raft_proto.display());
        write_empty_raft_rs();
        return;
    }
    ...
    tonic_build::configure()
        .build_client(true)
        .build_server(true)
        .compile_protos(&["proto/raft/raft.proto"], &["proto"] )
        .unwrap_or_else(|e| panic!("Failed to compile raft proto: {}", e));
}

fn write_empty_raft_rs() {
    let out_dir = env::var("OUT_DIR").expect("OUT_DIR not set");
    let raft_rs = Path::new(&out_dir).join("raft.rs");
    let mut f = File::create(&raft_rs).expect("failed to create fallback raft.rs");
    f.write_all(b"// fallback generated by build.rs when raft proto is unavailable\n")
        .expect("failed to write fallback raft.rs");
}
